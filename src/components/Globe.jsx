...
// scripts/build-globe-data.mjs — see that script for why (this globe
// re-projects every country every animation frame, so path complexity
// is a per-frame cost, not a one-time load cost).
...
export default function Globe({ className }) {
  ...
  useEffect(() => {
    ...
    // Rotates the globe to face (targetLon, targetLat) exactly — both
    // axes, not just longitude — so every country ends up dead center
    // on the visible face regardless of hemisphere. Longitude still
    // takes the shortest path around the ±180° wrap; latitude never
    // wraps so it's a plain lerp. Without the latitude component,
    // southern-hemisphere countries (Brazil, Argentina) always landed
    // low on the disc — since only longitude was ever corrected and
    // latitude stayed pinned at CENTER.lat — low enough to end up
    // behind the hiring-flow card below the globe. Reduced motion
    // skips the tween and jumps straight there.
    function rotateTo(targetLon, targetLat, onDone) {
      if (reduceMotion) {
        currentLon = targetLon;
        currentLat = targetLat;
        setRotationLon(targetLon);
        setRotationLat(targetLat);
        onDone();
        return;
      }
      const fromLon = currentLon;
      const fromLat = currentLat;
      const delta = ((targetLon - fromLon + 540) % 360) - 180;
      const toLon = fromLon + delta;
      const start = performance.now();
    ...
    function takeTurn() {
      if (cancelled) return;
      rotateTo(pins[idx].lon, pins[idx].lat, () => {
        if (cancelled) return;
        setActiveIndex(idx);
        timers.push(
          setTimeout(() => {
  ...
  const { landPath, graticulePath, outlinePath, points } = useMemo(() => {
    projection.rotate([-rotationLon, -rotationLat]);
    ...
    const visiblePoints = candidates
      .map((p) => {
        const distance = geoDistance([p.lon, p.lat], [rotationLon, rotationLat]);
  ...
  }, [projection, countries, graticule, rotationLon, rotationLat, spinDone, activeIndex, reduceMotion]);
      ...
      <AnimatePresence>
        {points.map((p) => (
          ...
          >
            ...
            <circle className="map-pin-pulse" cx={p.x} cy={p.y} r={5} />
            <circle className="map-pin-dot" cx={p.x} cy={p.y} r={2.5} />
            <text
